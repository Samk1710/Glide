...
// Extract user ID from request - accept from query params or body
const getUserId = (request: NextRequest, body?: any): string => {
  ...
  if (body && body.userId) return body.userId;
  ...
  try {
    ...
    switch (action) {
      ...
      case 'set_user_wallets': {
        const { agentAddress, wallets } = body;
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'session_status') {
      ...
      console.log('Getting session status for userId:', userId);
      
      const sessionManager = new AgentSessionManager();
      const hasSession = await AgentSessionManager.hasActiveSession(userId);
      const session = hasSession ? await AgentSessionManager.getSession(userId) : null;
