...
class AgentWalletManager {
  ...
  /**
   * Initialize agent wallet with 0xGasless integration and create/update session
   */
  private async initializeAgentWallet(config: AgentWalletConfig, existingSessionId?: string): Promise<AgentWallet> {
    try {
      const { privateKey, rpcUrl, apiKey, chainId, userId } = config;
      ...
      // Create or update session if not restoring from existing session
      let sessionId = existingSessionId;
      if (!existingSessionId) {
        const sessionResult = await AgentSessionManager.createSession(
          privateKey,
          address,
          userId,
          chainId
        ...
        if (!sessionResult.success) {
          console.warn('Failed to create session, wallet will not persist');
        } else {
      ...
      // Store the agent wallet in memory
      this.agents.set(address, agentWallet);

      console.log(`Agent wallet initialized: ${address} on chain ${chainId}${sessionId ? ` with session ${sessionId}` : ''}`);
      return agentWallet;
  ...
  /**
   * Check if user has an active session and restore if needed
   */
  async hasActiveSession(userId: string): Promise<boolean> {
    return await AgentSessionManager.hasActiveSession(userId);
  }
  ...
  /**
   * Clear agent session and remove from memory
   */
  async clearAgentSession(address: string, userId?: string): Promise<boolean> {
    const agent = this.agents.get(address);
    
    if (agent && agent.sessionId) {
      await AgentSessionManager.clearSession(userId, agent.sessionId);
    }
  ...
  /**
   * Remove agent wallet from memory (but keep session)
   */
  ...
  extendSession(): boolean {
    return AgentSessionManager.extendSession();
  }
}
